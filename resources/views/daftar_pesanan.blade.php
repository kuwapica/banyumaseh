            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($pesanans as $pesanan)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap font-mono font-bold">{{ $pesanan->kode_booking }}</td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">{{ \Carbon\Carbon::parse($pesanan->tanggal_kunjungan)->format('d M Y') }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $pesanan->jumlah_tiket }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">Rp {{ number_format($pesanan->harga_tiket, 0, ',', '.') }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">Rp {{ number_format($pesanan->total_bayar, 0, ',', '.') }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                    {{ $pesanan->status === 'confirmed' ? 'bg-green-100 text-green-800' : 
                                       ($pesanan->status === 'pending' ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800') }}">
